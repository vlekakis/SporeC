...
public class DBWrapper extends DB
{
     ...
     * Insert a record in the database. Any field/value pairs in the specified values HashMap will be written into the record with the specified
     * record key.
     * SPORE: Insert for Simple signature
     * @param table The name of the table
     * @param key The record key of the record to insert.
     * @param values A HashMap of field/value pairs to insert in the record
    ...
    public int insertSS(String table, String key, HashMap<String,ByteIterator> values, Signer sporeObj)
		...
		int res=_db.insert(table, key, values);
        en=System.nanoTime();
		measurementUtil(st, en, SporeStrings.SS_INSERT, 0);
	...
	public int updateSS(String table, String key, HashMap<String,ByteIterator> values, Signer sporeObj)
		...
		values = sporeObj.signFields(values);
		long en = System.nanoTime();
		measurementUtil(st, en, SporeStrings.SS_SIGN_ON_UPDATE, 0);

		st=System.nanoTime();
		int res=_db.update(table, key, values);
		en=System.nanoTime();
		measurementUtil(st, en, SporeStrings.SS_UPDATE, res);
		return res;
	}
	...
	public int readSS(String table, String key, Set<String> fields,
		...
		int res =_db.read(table, key, fields, result);
		long en = System.nanoTime();
		measurementUtil(st, en, SporeStrings.SS_READ, res);

		st = System.nanoTime();
		sporeObj.verifySignatureOnFields(result);
		en = System.nanoTime();
	 ...
	 * @param fields The list of fields to read, or null for all of them
	 * @param result A Vector of HashMaps, where each HashMap is a set field/value pairs for one record
	 * @param sporeObj Spore object to verify
	 * @return Zero on success, a non-zero error code on error
	 */
	public int scanSS(String table, String startkey, int recordcount, Set<String> fields,
		...
		int res = _db.scan(table, startkey, recordcount, fields, result);
		long en = System.nanoTime();
		measurementUtil(st, en, SporeStrings.SS_SCAN, res);

		Iterator<HashMap<String,ByteIterator>> it = result.iterator();
		st = System.nanoTime();
		while(it.hasNext()) {
			sporeObj.verifySignatureOnFields(it.next());
		}
		en = System.nanoTime();
		measurementUtil(st, en, SporeStrings.SS_VERIFY_ON_SCAN, 0);
		return res;
	}


	private void measurementUtil(long st, long en, String key, int res) {
		_measurements.measure(key, (int) ((en - st) / 1000));
		_measurements.reportReturnCode(key, res);
	}
